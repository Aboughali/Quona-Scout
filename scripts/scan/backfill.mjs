...
export const AFRICA_HINT = /\b(africa|african|nigeria|kenya|egypt|south africa|ghana|morocco|tanzania|uganda|rwanda|senegal|ethiopia|zambia|cameroon|tunisia|algeria|botswana|mozambique|lagos|nairobi|cairo|cape town|johannesburg|accra|kampala|dakar|abidjan|kigali)\b/i;
...
/**
 * Does the article's URL slug name the company?
 *
 * Publishers slugify the headline, so "/2024/06/connect-money-raises-8-million-seed-round"
 * identifies the subject even when the visible title is in another language -- which is exactly
 * the Wamda case, where the page title is Arabic and no name-matching on the title can work.
 */
function slugNames(url, name) {
  ...
  return new RegExp(`(?<![a-z0-9])${slug.replace(/-/g, '[-_]?')}(?![a-z0-9])`).test(path);
}
...
export function assessRelevance(item, company, source = null) {
  ...
  // The URL slug is identity evidence -- the publisher wrote the company's name into the address
  // of the article -- but ONLY for a distinctive name. For an ordinary word it is worthless:
  // "cobi-raises-1m-to-scale-customer-intelligence" and "amd-rack-scale-system" both carry
  // "scale" in the slug while having nothing to do with the company called Scale. It also stays
  // subject to the geography check, or the Canadian Float's slug reads as the South African one.
  if (
    slugNames(item.url, name) &&
    !isAmbiguousName(company.name) &&
    passesGeographyCheck(source, company, `${title} ${text}`)
  ) {
    return { keep: true, tier: 'url-slug', reason: 'named in the article URL' };
  }
  ...
  // back to the company.
  // A search snippet is a few dozen words chosen by the search engine; repetition inside it
  // says nothing about how much the article is about the company.
  const count = item.snippetOnly ? 0 : mentionCountExactCase(text, name);
