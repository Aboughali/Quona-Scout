...
import { AFRICA_HINT, isAmbiguousName } from './backfill.mjs';
...
export function enrichFindings(findings, companies) {
  ...
  for (const f of findings) {
    ...
    // A url-slug hint is re-judged here rather than trusted from the record: findings collected
    // before the slug rule was tightened carry that tier for ordinary-word names like Scale and
    // Float, where a slug proves nothing. Re-checking at enrich time un-attaches those without
    // deleting a single stored finding.
    const hintIsTrustworthy = f.backfill_tier !== 'url-slug' || !isAmbiguousName(f.backfill_query ?? '');
    if (f.backfill_query && hintIsTrustworthy && match.confidence < AUTO_ATTACH_CONFIDENCE) {
    ...
    // GEOGRAPHY SANITY CHECK. Company names collide across continents: this database's Float is
    // South African, but there is a Canadian Float raising CAD $85m and a European one closing a
    // Series A, and both say "Float raises..." in the headline exactly like the real one. The
    // backfill guard applies this test at collection time; findings that arrive any other way
    // would otherwise slip past it here. For an ordinary-word name, an article that places the
    // company nowhere in Africa is not safe to attach automatically -- it goes to Needs Review
    // instead of onto the company's profile.
    if (match.company && match.confidence >= AUTO_ATTACH_CONFIDENCE && isAmbiguousName(match.company.name)) {
      const hay = `${f.headline || ''} ${f.raw_content || ''}`;
      const placedInAfrica =
        AFRICA_HINT.test(hay) ||
        (match.company.country && new RegExp(`\\b${match.company.country.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i').test(hay));
      if (!placedInAfrica) {
        match = {
          ...match,
          confidence: Math.min(match.confidence, AUTO_ATTACH_CONFIDENCE - 0.05),
          rationale: `${match.rationale} Held back: "${match.company.name}" is also an ordinary word and this article places it nowhere in Africa -- likely a same-name company elsewhere.`,
        };
      }
    }
