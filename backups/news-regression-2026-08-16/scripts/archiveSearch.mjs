...
export async function searchArchive(
  source,
  query,
  { perPage = 100, maxBodies = 12, after = null, timeoutMs = 25000, looksRelevant = null, counters = null } = {}
) {
  ...
  const wanted = screened.slice(0, maxBodies).map((p) => p.id).filter((id) => id != null);
  if (counters) {
