...
export async function executeBackfill({ log = () => {}, dryRun = false, scope = 'watchlist', only = null, limit = null, ids = null } = {}) {
  ...
  // A backfill NEVER writes to the live store. It stages its output and stops. Promotion is a
  // separate, deliberate command (npm run scan:promote), so a run that is killed half way --
  // as the --scope=all run was on 2026-08-16 -- leaves the live store untouched by construction.
  ...
  if (!dryRun) {
    staged = await writeStagingRun({
      ...
      report: {
        ...
        companiesSearched: stats.companiesSearched,
        candidates: stats.ranked ?? null,
        rejected: stats.skippedIrrelevant ?? null,
        sourcesScanned: perSource.map((s) => s.name),
      },
    });
  }
  ...
  const run = {
    ...
    findingsAdded: 0,               // nothing reaches the live store until promotion
    existingFindings: existing.length,
    duplicates: stats.skippedDuplicate ?? 0,
    rejected: stats.skippedIrrelevant ?? 0,
    sourcesScanned: perSource.map((s) => s.name),
    stagingSucceeded: Boolean(staged),
    validationSucceeded: null,      // set by scan:verify / scan:promote
    promoted: false,
  ...
  executeBackfill({ log: (m) => console.log(m), dryRun, scope, only, limit, ids })
    .then((result) => {
      ...
      console.log(`\n${dryRun
        ...
        : `${result.findingsStaged} findings STAGED as ${result.runId}. The live store is unchanged (${result.finalLiveFindingCount} findings).`}`);
      if (!dryRun) {
        console.log('\nNext:');
        console.log(`  npm run scan:verify -- --run=${result.runId}     check what promotion would do`);
        console.log(`  npm run scan:promote -- --run=${result.runId}    merge it into the live store`);
        console.log(`  npm run scan:discard -- --run=${result.runId}    throw the batch away`);
      }
    })
    .catch((err) => {
      console.error(err);
